...
int insere_Pilha(Pilha *pi, struct aluno al) {
  if (pi == NULL ||
      pi->qtd == MAX)  // Verifica se não é um elemnto nulo e se não está cheia
    return 0;
...
int remove_Pilha(Pilha *pi, struct aluno *al) {
  if (pi == NULL ||
      pi->qtd == 0)  // Verifica se não é um elemento nulo e se há dados. if (pi
                     // == NULL || pi->qtd == 0) difere de if (pi->qtd == 0 ||
                     // pi == NULL), pois se pi for nulo, dará errado (crash)
    return -1;

  // Copiar elemento e remover
  // Não terá problema apenas retirar o "topo" da posição anterior, mesmo que o
  // dado esteja na posição antes de ser retirada, o próximo dado a ser
  // acrescentado será sobrescrito
  pi->qtd--;
