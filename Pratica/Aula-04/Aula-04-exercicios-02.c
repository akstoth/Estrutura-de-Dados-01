...
void estado_inicial(Pilha *p) {
  ...
  // Cuidado, se na criação nomes[5][5], se ficar com 5 ou mais, sobrepõe
  ...
  // remove um elemento:
  pop(p, &a0);  // Pop recebe um ponteiro para aluno, logo enviamos o endereço
                // com &. Retira o último
  // pop(p, &a0); // Retira o penúltimo
  top(p, &a0);  // Mostra quem está no topo
  // printf("nome: %s, idade: %d", a0.nome, a0.idade); // Com dois pop,
  // retira-se e ainda salva o último retirado, mas quem está no topo é ze
}
...
float media_menores(Pilha *p) {
  ...
  for (int i = 0; i < p->qtd; i++) {
    if (p->dados[i].idade <
        18) {  // Como temos um ponteiro p, com um array de structs dados,
               // usamos o indice no dados e como já estamos usando o ->, usamos
               // o .idade
...
void imprime_pilha(Pilha *p) {  // Basta enviar o ponteiro da pilha, onde é
